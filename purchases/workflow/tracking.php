...
// Factures par statut
$query = "SELECT status, COUNT(*) as count FROM purchase_invoices GROUP BY status";
$stmt = $pdo->prepare($query);
$stmt->execute();
$invoices_by_status = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
...
LEFT JOIN purchase_invoices pi ON po.id = pi.order_id
LEFT JOIN accounting_entries ae ON pi.id = ae.invoice_id
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="progress-timeline">
                            ...
                            <div class="text-center">
                                <div class="timeline-step <?= !empty($invoices_by_status) ? 'completed' : 'pending' ?>">
                                    <i class="fas fa-receipt"></i>
                                </div>
                                <small class="mt-2 d-block">Facturation</small>
                                <strong><?= array_sum($invoices_by_status ?: []) ?></strong>
                            </div>
                            
                            <div class="text-center">
                                <div class="timeline-step <?= !empty($invoices_by_status['paid']) ? 'completed' : 'pending' ?>">
                                    <i class="fas fa-calculator"></i>
                                </div>
                                <small class="mt-2 d-block">Comptabilité</small>
                                <strong><?= $invoices_by_status['paid'] ?? 0 ?></strong>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-header">
                                <h6 class="mb-0"><i class="fas fa-receipt me-2"></i>Factures</h6>
                            </div>
                            <div class="card-body">
                                <?php foreach (['received' => 'Reçues', 'validated' => 'Validées', 'approved' => 'Approuvées', 'paid' => 'Payées'] as $status => $label): ?>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span><?= $label ?></span>
                                        <strong><?= $invoices_by_status[$status] ?? 0 ?></strong>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        <?php foreach ($workflow_items as $item): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?= htmlspecialchars($item['requester_name']) ?>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    ...
                                                    if ($item['invoice_id']) {
                                                        $progress = 90;
                                                        $current_step = 'Facture reçue';
                                                    }
                                                    if ($item['accounting_status'] === 'posted') {
                                                        $progress = 100;
                                                        $current_step = 'Comptabilisé';
                                                    }
                                                    ?>
                                                ...
                                                <td>
                                                    ...
                                                    } elseif ($item['invoice_status']) {
                                                        echo '<span class="badge bg-' . ($status_colors[$item['invoice_status']] ?? 'secondary') . '">Facture ' . $item['invoice_status'] . '</span>';
                                                    } elseif ($item['order_status']) {
                                                ...
                                                <td>
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="../requests/view.php?id=<?= $item['request_id'] ?>" class="btn btn-outline-primary" title="Voir demande">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                        <?php if ($item['order_id']): ?>
                                                            <a href="../orders/view.php?id=<?= $item['order_id'] ?>" class="btn btn-outline-success" title="Voir commande">
                                                                <i class="fas fa-file-invoice"></i>
                                                            </a>
                                                        <?php endif; ?>
                                                        <?php if ($item['invoice_id']): ?>
                                                            <a href="../invoices/view.php?id=<?= $item['invoice_id'] ?>" class="btn btn-outline-warning" title="Voir facture">
                                                                <i class="fas fa-receipt"></i>
                                                            </a>
                                                        <?php endif; ?>
                                                    </div>
